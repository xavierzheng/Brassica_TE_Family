...
ggplot()+
  ...
  coord_cartesian(
    ylim = quantile(df_temp$ACCUM_METHYL_PERC, probs = c(0.05, 0.95), na.rm = T)
  )+
  theme_classic()+
  theme(
    axis.line = element_line(
...
fwrite(
  df_draw, 
  file = "Gypsy_RELATION_GENE_TE.txt", 
...
fwrite(
  df_draw, 
  file = "Gypsy_RELATION_OTHERTE_TE.txt", 
  col.names = T, sep = "\t", row.names = F, quote = F
)
...
ggplot()+
  ...
  coord_cartesian(
    ylim = c(1.4, 4.6)
    #ylim = quantile(log10(df_temp$CLOSEST_OTHERTE_DISTANCE), probs = c(0.05, 0.99), na.rm = T)
  )+
  theme_classic()+
  theme(
    axis.line = element_line(
...
ggplot()+
  ...
  geom_rect(
    ...
    mapping = aes(
      xmin = -Inf,
      xmax = Inf,
      ymin = -Inf,#quantile(df_temp$COUNT, probs = 0.1, na.rm = T),
...
cowplot::ggsave2(
  plot = COMBINE_PLOT,
  filename = "/Fig_4_TE_Feature/FullInfo_LTR_Gypsy.pdf", 
  width = 4.5,
  height = 9.7
)
