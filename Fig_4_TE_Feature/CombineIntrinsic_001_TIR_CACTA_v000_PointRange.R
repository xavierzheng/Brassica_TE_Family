...
df_draw <- df_temp %>%
  ...
  summarise(
    ...
    Q25 = quantile(log10(COUNT), probs = 0.25), 
    Q75 = quantile(log10(COUNT), probs = 0.75)
  ) %>%
  ungroup() 

ggplot()+
  geom_hline(
    yintercept = quantile(log10(df_temp$COUNT), probs = c(0.5)), 
    linetype = "solid", 
    color = "#bababa",
    linewidth = 0.2
  )+
  geom_rect(
    ...
    mapping = aes(
      ...
      xmax = Inf,
      ymin = -Inf,
      ymax = quantile(log10(COUNT), probs = 0.1)
    ), 
    fill = "#1ecbe1", 
    alpha = 0.5
  )+
  geom_rect(
    ...
    mapping = aes(
      xmin = -Inf,
      xmax = Inf,
      ymin = quantile(log10(COUNT), probs = 0.9),
      ymax = Inf
    ), 
    fill = "#edd612", 
    alpha = 0.5
...
df_draw<- df_temp %>%
  ...
  summarise(
    ...
    Q25 = quantile(TE_LEN, probs = 0.25), 
    Q75 = quantile(TE_LEN, probs = 0.75)
  ) %>%
  ungroup()

ggplot()+
  geom_hline(
    yintercept = quantile(df_temp$TE_LEN/1000, probs = c(0.5)), 
    linetype = "solid", 
    color = "#bababa",
    linewidth = 0.2
  )+
  geom_rect(
    ...
    mapping = aes(
      ...
      ymax = quantile(TE_LEN/1000, probs = 0.1)
    ), 
    fill = "#1ecbe1", 
    alpha = 0.5
  )+
  geom_rect(
    ...
    mapping = aes(
      xmin = -Inf,
      xmax = Inf,
      ymin = quantile(TE_LEN/1000, probs = 0.9),
      ymax = Inf
    ), 
    fill = "#edd612", 
    alpha = 0.5
  ...
  scale_y_continuous(
    breaks = round(unname(quantile(df_temp$TE_LEN/1000, probs = c(0.1, 0.5, 0.9))),1),
    expand = expansion(mult = c(0, 0), add = c(0, 0))
  )+
  coord_cartesian(
    ylim = c(0.5, NA)
...
print("# GC content of full region ===========================================")
#glimpse(df_gc)
rm(df_temp)
df_temp <- df_gc %>%
  left_join(
...
ggplot()+
  ...
  coord_cartesian(
    #ylim = c(10,60)
    ylim = quantile(df_temp$GC*100, probs = c(0.001, 0.999))
  )+
  theme_classic()+
  theme(
    axis.line = element_line(
...
ggplot()+
  ...
  coord_cartesian(
    #ylim = c(30, 70)
    ylim = quantile(df_temp$ACCUM_METHYL_PERC, probs = c(0.001, 0.99))
  )+
  theme_classic()+
  theme(
    axis.line = element_line(
...
ggplot()+
  ...
  coord_cartesian(
    #ylim = c(20, 30)
    ylim = quantile(df_temp$ACCUM_METHYL_PERC, probs = c(0.001, 0.999))
  )+
  theme_classic()+
  theme(
    axis.line = element_line(
...
# fwrite(
#   df_draw, 
#   file = "CACTA_RELATION_GENE_TE.txt", 
...
fwrite(
  df_draw, 
  file = "CACTA_RELATION_OTHERTE_TE.txt", 
  col.names = T, sep = "\t", row.names = F, quote = F
)
...
ggplot()+
  ...
  coord_cartesian(
    ylim = c(1.3, 4)
    #ylim = quantile(df_temp$CLOSEST_OTHERTE_DISTANCE/1000, probs = c(0, 0.99))
  )+
  theme_classic()+
  theme(
    axis.line = element_line(
...
# COMBINE_PLOT <- plot_outer2k_GC/plot_dist_TE/plot_relation_TE/plot_dist_gene/plot_relation_gene/plot_len/plot_count/plot_share/plot_heat + 
#   patchwork::plot_layout(heights = c(3, 3, 3, 3, 3, 3, 3, 0.5, 5))

COMBINE_PLOT

cowplot::ggsave2(
  plot = COMBINE_PLOT,
  filename = "/Fig_4_TE_Feature/FullInfo_TIR_DTC.pdf", 
  width = 7.2,
  height = 7
)
