...
{if ($3 == Class ) {split($9,x,"ID=");split(x[2],y,";");split(y[1],z,"_");NUM=z[3];\
if(z[2]!="homo"){ID=TE}\
else if(z[2]=="homo"){ID="TE_homo"};\
...
# make a dictionary and replace fasta
Dict=$(echo "/nfs/project1/Repeat/10_integrate/06_clusterTE/CC/02-strucTEfa/strucTE_fasta."$Genome"_"$TE".dict")
less ${Bed} | awk 'BEGIN{FS="\t";OFS="\t"}{split($4,a,";");\
print a[3]"::"$1":"$2"-"$3"("$6")", a[1]"::"$1":"$2"-"$3"("$6")"}' > $Dict

Fasta=$(echo "/nfs/project1/Repeat/10_integrate/07_homo80/strucTE."$TE".fa")
less $Fasta | \
awk -v Genome=$Genome -v TE=$TE 'BEGIN{FS="\t";OFS="\t"}\
{if(NR==FNR){d[">"$1]=">"$2} else {ID=">DNA/"TE"::"Genome"_";if($0 ~ ID){$0=d[$0]}print}}' $Dict - > tmp
mv tmp /nfs/project1/Repeat/10_integrate/06_clusterTE/CC/03-mcl/all5.struc_$TE.fa
...
print a[1],b[1]}' > $Genome.$TE.TE_family.dict.tmp
